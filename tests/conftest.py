...
@pytest.fixture(scope="session", autouse=True)
def cluster_load(
    request, pvc_factory_session, service_account_factory_session, pod_factory_session
):
    """
                ...
                try:
                    ...
                    if config.RUN.get('io_in_bg'):
                        ...
                        if latency > 1 and len(cl_load_obj.dc_objs) > 0:
                            log.warning(
                                "Latency is higher than 1 second. Lowering IO load"
                                " by deleting an FIO pod that is running in the "
                                "test background. Once the latency drops back to "
                                "less than 0.5 seconds, FIO pod will be re-spawned"
                            )
                            cl_load_obj.delete_pod_and_pvc()
                        ...
                        while latency < 0.5 and diff > 0 and (
                            ...
                            log.info(
                                "Latency is lower than 0.5 seconds. "
                                "Re-spinning FIO pod"
                            )
                            cl_load_obj.create_fio_pod()
                            latency = cl_load_obj.calc_trim_metric_mean(
                                constants.LATENCY_QUERY
                            )
                            diff -= 1
